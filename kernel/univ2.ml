...
let constraint_add_leq k v u c =
  (* We just discard trivial constraints like u<=u *)
  if Expr.equal v u && k >= 0 then c   (* ??? *)
  else 
    match v, u with
    | (x,n), (y,m) ->
    let j = m - n + k in
       if Level.equal x y then
         if j < 0 then 
           raise (UniverseInconsistency (Le n, Universe.tip v, Universe.tip u, None))
         else c
       else Constraint.add (x,Le j,y) c
      (*      let j = m - n in *)
...
let check_univ_leq_one k u v = Universe.exists (Expr.leq u) v   (* todo *)

let check_univ_leq k u v = 
  Universe.for_all (fun u -> check_univ_leq_one k u v) u

let enforce_leq_n n u v c =
  let open Universe.Huniv in
  let rec aux acc v =
  match v with
  | Cons (v, _, l) ->
    aux (fold (fun u -> constraint_add_leq n u v) u c) l
  | Nil -> acc
  in aux c v

let enforce_leq_n n u v c =
  if check_univ_leq n u v then c
  else enforce_leq_n n u v c

let enforce_leq = enforce_leq_n 0

let enforce_leq_level_n n u v c =
  (* if Level.equal u v then c else *) Constraint.add (u,Le n,v) c

let enforce_leq_level = enforce_leq_level_n 0

let enforce_univ_constraint (u,d,v) =
  match d with
  | Eq -> enforce_eq u v
  | Le n -> enforce_leq_n n u v
...
let explain_universe_inconsistency prl (o,u,v,p) =
  ...
  let pr_rel = function
    | Eq -> str"=" | Le n -> str("<=(" ^ string_of_int n ^ ")")
  in
